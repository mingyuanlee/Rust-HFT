...
// Two Trees: Buy Tree and Sell Tree
// Each has ownership to orders and limits, instead of the overall one map for both
...
struct LNode {
  limit_price: u64,
  size: u64, // total shares
  total_vol: u64, // total shares * price
  parent: Option<u64>,
  order_count: u64,
...
struct LNodeTree {
  ...
  root: Option<u64>,
  // oid => Order
  orders_map: HashMap<u64, Order>,
  // price => LNode because price is unique, can be used as key
  limits_map: HashMap<u64, LNode>,
  best_limit: Option<u64>, // best buy/sell limit price
  next_order_id: u64,
}

impl LNodeTree {
  fn get_mut_limit(&self, )

  // Assume the order already put in the ownership map
  // LNode linked list structure
  // (1) empty case: headp and tailp both null
  // (2) single node case: headp and tailp both point to the single node
  // (3) multi node case: headp and tailp different
  fn push_order(&self, lnode: &mut LNode, o: &mut Order) {
    let oid = o.oid;
    if lnode.headp == None && lnode.tailp == None {
      lnode.headp = Some(oid);
      lnode.tailp = Some(oid);
    } else if (lnode.headp == lnode.tailp) {
      lnode.headp = Some(oid);
      let mut tailo = lnode.tailp
    }
  }

  // called when there is a new order but there's no existing limit price
  fn insert_limit(&mut self, shares: u64, limit_price: u64) {
    let mut tree_node = LNode { 
      limit_price, 
      size: shares, 
      total_vol: shares * limit_price, 
      parent: None,
      order_count: 1,
      left_child: None,
      right_child: None,
    ...
    // let order = 
    // push order to limit node
  }

  fn find_largest_predecessor(&self, )
}
...
// fn drain_limit(shares_to_execute: u64, best_sell_limit: &mut LNode) -> (u64, bool) {
...
//   // 1.1 Get the best sell LNode price best_sell_price (we keep track of this LNode object in the OrderBook struct so O(1)). Note this is always the smallest price in the sell tree, which is physically the leftmost node.
...
//   // 1.4.1 Find its largest predecessor, set as the best_sell_price in OrderBook struct, delete the LNode
//   // 1.5 If shares_to_execute == 0, means execution is done, return
//   // 1.6 If shares_to_execute > 0, means we need to add this one to the buy tree, add the new Order at the LNode price, call add_order()
// }
...
//   // u64 => LNode
//   let limits_ownership_map: HashMap<u64, LNode> = HashMap::new();
